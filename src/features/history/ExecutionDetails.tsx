...
export function ExecutionDetails({ details }: { details: Details }) {
  ...
  return (
    <aside className="silver-card history-details" aria-labelledby="execution-details-title">
      <header><div><span className="eyebrow">脱敏记录</span><h2 id="execution-details-title">执行详情</h2></div><span className={`history-status history-status--${record.status}`}>{record.status}</span></header>
