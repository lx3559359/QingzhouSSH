...
- [ ] 先写当前版本、数据目录、自动检查、来源、更新详情和最新状态组件测试。
- [ ] 用白银渐变、强阴影和清晰文字替换设置占位页。
- [ ] 提交：`feat: add dimensional update settings`

## Task 9：实现下载、确认和错误 UX

- [ ] 先写镜像提示、进度、下载失败、签名/哈希拒绝、安装二次确认和清理测试。
- [ ] 安装确认明确说明应用将退出；不提供静默自动安装。
- [ ] 提交：`feat: control update download and install`
